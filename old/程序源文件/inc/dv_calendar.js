...
function dateSelector()  //构造dateSelector对象，用来实现一个日历形式的日期输入框。
{
  ...
  this.defvalue='';//定义文本框的默认值。
}
function display()  //定义dateSelector的display方法，它将实现一个日历形式的日期选择框。
{
  var week=new Array('日','一','二','三','四','五','六');
  document.write("<style type=text/css>");
  document.write("  .ds_font td { font: normal 12px 宋体; color: #000000; }");
  ...
  document.write("<button style='width:60px;height:23px;font-size:12px;vertical-align:middle;margin:1px;border:1px solid #A4B3C8;background-color:#DFE7EF;' type=button onclick=showdiv(this.nextSibling) onfocus=this.blur()>选择日期</button>");
 ...
 for(j=0;j<7;j++)
  ...
  document.write("    </table>");
  document.write("    <span style=cursor:hand onclick=showdiv(this.parentNode.parentNode)>【关闭】</span>");
  document.write("  </div>");
  document.write("</div>");
